import { isObject, createInstanceofPredicate, getNextId, makeNonEnumerable, Lambda, EMPTY_ARRAY, addHiddenFinalProp, addHiddenProp, invariant, enumerable } from "../utils/utils";
...
class ObservableArrayAdministration<T> implements IInterceptable<IArrayWillChange<T> | IArrayWillSplice<T>>, IListenable {
	...
	constructor(name: string, enhancer: IEnhancer<T>, public array: IObservableArray<T>, public owned: boolean) {
...
export class ObservableArray<T> extends Array<T> {
	...
	findIndex(predicate: (item: T, index: number, array: ObservableArray<T>) => boolean, thisArg?: any, fromIndex = 0): number {
	...
	move(fromIndex: number, toIndex: number): void {
		function checkIndex(this: ObservableArray<T>, index: number) {
			if (index < 0) {
	...
	set length(value: number) {
		this.$mobx.setArrayLength(value);
	}
}
...
function createArrayEntryDescriptor(index: number) {
	return {
		enumerable: false,
		configurable: false,
		get: function (this: ObservableArray<any>) {
			// TODO: Check `this`?, see #752?
			return this.get(index);
		},
		set: function (this: ObservableArray<any>, value: any) {
